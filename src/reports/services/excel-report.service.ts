...
export class ExcelReportService {
  ...
  private async buildStudentReport(
    ...
    summarySheet.addRow([
      'Total Score:',
      `${data.summary.totalScore} out of ${data.summary.totalPossibleScore}`,
    ]);
    ...
    if (data.testResults && data.testResults.length > 0) {
      data.testResults.forEach((result: any) => {
        ...
        const finishDate = result.finishedAt
  ...
  private async buildSubjectReport(
    workbook: ExcelJS.Workbook,
    data: any,
    report: Report,
  ): Promise<void> {
    ...
    const summarySheet = workbook.addWorksheet('Summary');
    workbook.addWorksheet('Test Performance');
    workbook.addWorksheet('Student Results');
  ...
  private async buildCourseReport(
    workbook: ExcelJS.Workbook,
    data: any,
    report: Report,
  ): Promise<void> {
    ...
    courseSheet.addRow([
      'Course-specific report details would be included here.',
    ]);
  }

  private async buildPackageReport(
    workbook: ExcelJS.Workbook,
    data: any,
    report: Report,
  ): Promise<void> {
    ...
    packageSheet.addRow([
      'Package-specific report details would be included here.',
    ]);
  }

  private async buildTestReport(
    workbook: ExcelJS.Workbook,
    data: any,
    report: Report,
  ): Promise<void> {
  ...
  private async buildInstituteReport(
    workbook: ExcelJS.Workbook,
    data: any,
    report: Report,
  ): Promise<void> {
    ...
    instituteSheet.getRow(instituteSheet.rowCount).font = {
      bold: true,
      size: 14,
    };
    instituteSheet.addRow([
      'Institute-specific report details would be included here.',
    ]);
  }

  private async buildOverallReport(
    workbook: ExcelJS.Workbook,
    data: any,
    report: Report,
  ): Promise<void> {
    ...
    overallSheet.addRow([
      'Overall system report details would be included here.',
    ]);
  }
}
